...
from khaleesi.proto.core_sawmill_pb2 import DESCRIPTOR, AppCallData
...
class Service(Servicer):
  ...
  def GetAppCallData(self, request: EmptyRequest, _: grpc.ServicerContext) -> AppCallData :
    """Get the call data of the calling service."""
    owner = request.requestMetadata.grpcCaller
    LOGGER.info(f'Getting site registry call data for {owner.site}-{owner.app}.')
    return SITE_REGISTRY.getCallData(owner = owner)
