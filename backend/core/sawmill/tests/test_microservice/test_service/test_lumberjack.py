...
class LumberjackServiceTestCase(SimpleTestCase):
  ...
  def _execute_successful_logging_test(
    ...
    self.assertEqual(LogResponse.ResultType.SUCCESS, result.result)
  ...
  def _execute_logging_test_with_parsing_error(
    ...
    # Assert result.
    logging.assert_called_once()
    #self.assertEqual(LogResponse.ResultType.PARSING_ERROR, result.result)
    self.assertEqual(expected_result.meta_logging_errors, result.errors)
  ...
  def _execute_logging_test_with_fatal_error(
    ...
    # Assert result.
    logging.assert_called_once()
    self.assertEqual(LogResponse.ResultType.FATAL_ERROR, result.result)
